...
```
The first dataset contains multiple variables regarding the amount and types of trash and collected by Mr. Trash Wheel and number of homes powered by the energy created. The dimensions of this data frame are `r dim(trash_wheel_df)` and there are `r (nrow(trash_wheel_df)) -1` total observations, with the last row containing variable totals. The second data frame contains information about precipitation in 2016 and 2017 and has dimensions of `r dim(prcp_df)`. There are `r filter(prcp_df, year == 2017) %>% select(total) %>% filter( !is.na(total)) %>% nrow()` months worth of data for 2017 and `r filter(prcp_df, year == 2016) %>% select(total) %>% nrow()` for 2016. The total precipitation for 2017 was `r filter(prcp_df,  year == 2017) %>% select(total) %>% sum()`. The median number of sports balls collected was `r median(trash_wheel_df$sports_balls)`.
...
This dataset contains information from `r nrow(distinct(brfss_df, locationdesc))` distinct locations and `rnrow(distinct(brfss_df, locationabbr))` distinct states (including DC). `r state_freq = count(brfss_df, locationabbr)` `r abbr2state(state_freq[which.max(state_freq$n), 1])` is the state with the most observations.
